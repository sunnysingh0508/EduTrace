        ...
        <div className="space-y-6 pb-20">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex items-center gap-4 transition-colors">
                    ...
                    <div>
                        <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">Avg. Attendance</p>
                        <h3 className="text-2xl font-bold text-slate-800 dark:text-slate-100">87.5%</h3>
                        <p className="text-xs text-green-500 font-medium flex items-center gap-1">
                            +2.4% <span className="text-gray-400 dark:text-slate-500">vs last month</span>
                        </p>
                    </div>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex items-center gap-4 transition-colors">
                    ...
                    <div>
                        <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">Syllabus Covered</p>
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 flex items-center gap-4 transition-colors">
                    ...
                    <div>
                        <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">Total Students</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="font-bold text-slate-800 dark:text-slate-100">Attendance Trend</h3>
                        <select className="text-sm border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg text-gray-500 dark:text-slate-300 focus:ring-primary-500 outline-none p-1">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h3 className="font-bold text-slate-800 dark:text-slate-100">Attention Needed</h3>
                    </div>
                    <p className="text-sm text-gray-500 dark:text-slate-400 mb-4">Students with attendance below 75% needs intervention.</p>

                    <div className="space-y-4">
                        {lowAttendanceStudents.map((student) => (
                            <div key={student.id} className="flex items-center justify-between p-3 bg-red-50/50 dark:bg-red-900/10 rounded-xl border border-red-100 dark:border-red-900/30">
                                <div className="flex items-center gap-3">
                                    <img src={student.avatar} alt={student.name} className="w-10 h-10 rounded-full" />
                                    <div>
                                        <h4 className="text-sm font-semibold text-slate-800 dark:text-slate-200">{student.name}</h4>
                                        <p className="text-xs text-red-500 dark:text-red-400 font-medium">Low Attendance</p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <span className="text-lg font-bold text-slate-800 dark:text-slate-200">{student.attendance}%</span>
                                </div>
                            </div>
                        ))}
                    </div>
                    <button className="w-full mt-6 py-2.5 text-sm text-gray-600 dark:text-slate-300 bg-gray-50 dark:bg-slate-700/50 hover:bg-gray-100 dark:hover:bg-slate-700 rounded-xl font-medium transition-colors">
                        View All Students
                    </button>
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 transition-colors">
                <h3 className="font-bold text-slate-800 dark:text-slate-100 mb-6">Curriculum Progress by Unit</h3>
                <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={curriculumData} layout="vertical" barSize={20}>
                            ...
                            <Bar dataKey="completed" fill="#8b5cf6" radius={4} background={{ fill: 'transparent', radius: 4 }} />
                        </BarChart>
                    </ResponsiveContainer>
                </div>
            </div>
